...
- **Lineups carry forward automatically at lock.** `autoFillCarryForward(md)` (lib/lineup/carry-forward.ts) copies a manager's most recent prior lineup into matchday N once N locks, for anyone who didn't set one. Wired into `sweepMatchday` (start) + the cron (`carryForwardLockedMatchdays`). Managers with NO prior lineup stay unscored (league rule = "carry forward last lineup only", not auto-generate a default XI). The manual "copy from last MD" button in the lineup builder is for pre-lock tweaks. Symptom this fixed: most managers showed 0 because `autoFillFromPriorMatchday` only ran from that manual button and never fired at lock.
